...
Documentation for this on-prem product has been moved from IBM Cloud to IBM Documentation at [https://www.ibm.com/docs/en/SSVKZ7_2.5.2/ibp-console-advanced-deployment.html](https://www.ibm.com/docs/en/SSVKZ7_2.5.2/ibp-console-advanced-deployment.html){: external}. Update your bookmarks.
{: important}
...
1. **Deploy an HSM**. Utilize the HSM appliance that is available in [{{site.data.keyword.cloud_notm}}](https://cloud.ibm.com/catalog/infrastructure/hardware-security-module){: external} or configure your own HSM. Record the value of the HSM `partition` and `PIN` to be used in the subsequent steps.
    ...
    - If you want to use [openCryptoki HSM](https://www.ibm.com/docs/en/linux-on-systems?topic=opencryptoki-what-is){: external} with your Z environment, or with any HSM that uses a **daemon**, see [Configure an HSM daemon](#ibp-console-adv-deployment-hsm-daemon) and then proceed to Part 2 **Configure an HSM client image**.
